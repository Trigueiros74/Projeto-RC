...
int cmd_myevents(char *request, int fd_udp, struct addrinfo *res) {
    ...
    if(strcmp(status, "OK") == 0) {
        /* Print each event on its own line. Payload is pairs: EID state */
        ...
        if (payload == NULL) {
            ...
            char *p = payload + 1;
            char eid[8]; int state; int consumed = 0;
            int any = 0;
            while (*p) {
                if (sscanf(p, " %7s %d %n", eid, &state, &consumed) == 2 && consumed > 0) {
                    printf("%s %d\n", eid, state);
                    p += consumed;
                    any = 1;
                } else break;
...
int cmd_reserve(char *request, struct addrinfo *res) {
    ...
    if(strcmp(status, "ACC") == 0) {
        ...
        unsigned int remaining_local = 0;
        if(sscanf(response, "%*s %u", &remaining_local) == 1) {
