...
int parse_create(char *buffer, char *request, char *UID) {
    ...
    // Accept user input as YYYY-MM-DD and convert to MM-DD-YYYY for server
    if(strlen(event_date_in) != 10 || event_date_in[4] != '-' || event_date_in[7] != '-') {
        fprintf(stderr, "Invalid date format! Use YYYY-MM-DD.\n");
        return 1;
    }
    // event_date_out = MM-DD-YYYY
    snprintf(event_date_out, sizeof event_date_out, "%.2s-%.2s-%.4s", event_date_in + 5, event_date_in + 8 - 3, event_date_in);
    // The above is a bit fragile; do explicit copy
    // Better: parse components
    char yyyy[5], mm[3], dd[3];
    yyyy[0]=event_date_in[0]; yyyy[1]=event_date_in[1]; yyyy[2]=event_date_in[2]; yyyy[3]=event_date_in[3]; yyyy[4]='\0';
    mm[0]=event_date_in[5]; mm[1]=event_date_in[6]; mm[2]='\0';
    dd[0]=event_date_in[8]; dd[1]=event_date_in[9]; dd[2]='\0';
    snprintf(event_date_out, sizeof event_date_out, "%s-%s-%s", mm, dd, yyyy);
