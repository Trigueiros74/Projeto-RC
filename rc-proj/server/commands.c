...
/* Basic lexicographic date comparator (mm-dd-yyyy). Returns:
   -1 if date1 < date2 (date1 in past), 0 if equal, 1 if date1 > date2.
   This is simplistic (compares yyyy-mm-dd after reformatting) but OK for tests.
*/
static int cmp_date(const char *d1, const char *d2) {
    if (!d1 || !d2) return 0;
    char b1[11], b2[11];
    if (strlen(d1) != 10 || strlen(d2) != 10) return 0;
    /* convert mm-dd-yyyy -> yyyy-mm-dd in temp buffers */
    snprintf(b1, sizeof b1, "%.4s-%.2s-%.2s", d1+6, d1+0, d1+3);
    snprintf(b2, sizeof b2, "%.4s-%.2s-%.2s", d2+6, d2+0, d2+3);
...
/* For quick tests we set "current date" to a fixed value.
   You can change this constant to simulate dates in the past/future for events.
*/
static const char *CURRENT_DATE = "11-12-2025"; /* mm-dd-yyyy */
